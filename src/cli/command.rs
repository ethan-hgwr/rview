...
pub(crate) struct CameraArgs {
    /// Initial yaw of the camera in radians (might get overriden depending on the choosen mode)
    #[arg(short, long, default_value_t = CameraState::DEFAULT_YAW)]
    pub(crate) yaw: f32,

    /// Initial pitch of the camera in radians (might get overriden depending on the choosen mode)
